...
export async function lean<D extends Document>(document: any): Promise<D> {
    try {
  ...
  export function leanObject<D extends any>(doc): D {
    try {
      if (doc && doc["_id"]) {
        doc.id = doc["._id"];
        delete doc["_id"];
...
export function isEmpty(val): boolean {
  ...
  export const isNumber = (item): boolean => typeof item === "number";
...
export function isValidDate(data) {
  ...
  export function isPlatformService(nodeEnv) {
  ...
  export function isArrayOfStringsAndNulls(arr: string[]) {
    if (Array.isArray(arr)) {
...
export function getEnumKeyOrValue(enums: any, enumKeyOrValue: any): string {
  ...
  export const getUniqueData = (data, searchKey) => {
  ...
  export function isSubset(arr: any[], arr2: any[]) {
